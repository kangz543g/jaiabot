...
export function createDivePacketFeature(map: Map, task_packet: TaskPacket) {
    ...
    const feature = createMarker(map, {
        title: "dive",
        lon: dive?.start_location?.lon ?? 0,
        lat: dive?.start_location?.lat ?? 0,
    });
...
function compareFeatures(
    ...
    newFeature: Feature<Geometry> | null,
) {
    if (
        selectedFeature?.get("botId") === newFeature?.get("botId") &&
        selectedFeature?.get("startTime") === newFeature?.get("startTime")
    ) {
